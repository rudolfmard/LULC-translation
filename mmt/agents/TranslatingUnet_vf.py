...
class TranslatingUnetVfAgent(BaseAgent):
    def __init__(self, config):
        ...
        # define data_loader
        self.data_loader = LandcoverToLandcoverDataLoader(
            config=config, device=self.device, pos_enc=True
        )
        ...
        if (
            ...
            self.models = EncDec(
                input_channels[0],
                output_channels[1],
                self.real_patch_sizes[0],
                self.real_patch_sizes[1],
            )

        # define optimizer
        self.optimizers = optim.Adam(
            self.models.parameters(), lr=self.config.learning_rate[0]
        )
        ...
        if self.config.tensorboard:
            self.summary_writer, self.tensorboard_process = tensorboard_summary_writer(
                config, comment=self.config.exp_name
            )
        ...
        for epoch in range(1, self.config.max_epoch + 1):
            ...
            if epoch > 1 and epoch >= 2 * self.config.validate_every:
                plt_loss()(
                    ...
                    savefig=join(self.config.out_dir, "loss.png"),
                )
    ...
    def train_one_epoch(self):
        ...
        for data in self.data_loader.train_loader[self.config.datasets[0]][
            ...
            plot_loss[self.config.datasets[0]].append(
                [self.current_iteration, loss.item()]
            )
            plot_loss[self.config.datasets[1]].append(
                [self.current_iteration, loss.item()]
            )
        ...
        if self.config.tensorboard:
            self.summary_writer.add_scalars(
                "Loss", {"training_loss": np.mean(plot_loss)}, self.current_epoch
            )
    ...
    def validate(self):
        ...
        with torch.no_grad():
            im_save = {
                d: {j: 0 for j in self.config.datasets} for d in self.config.datasets
            }
            for data in self.data_loader.valid_loader[self.config.datasets[0]][
                self.config.datasets[1]
            ]:
                # print(compt/self.data_loader.train_iterations)
                # compt+=1
        ...
        if self.config.tensorboard:
            self.summary_writer.add_scalars(
                "Loss", {"validation_loss": np.mean(plot_loss)}, self.current_epoch
            )
            self.summary_writer.add_image(
                "train/generated_image", out_img.transpose(2, 0, 1), self.current_epoch
            )
        return plot_loss
    ...
    def test(self):
        with torch.no_grad():
            ...
            res_oa = {
                d: {j: [0, 0] for j in self.config.datasets}
            ...
            conf_matrix = {
                d: {
                    ...
                    for j in self.config.datasets
                }
                for d in self.config.datasets
            }

            for data in self.data_loader.test_loader[self.config.datasets[0]][
                self.config.datasets[1]
            ]:
                # print(compt/self.data_loader.train_iterations)
                # compt+=1
                ...
                conf_matrix[self.config.datasets[1]][
                    self.config.datasets[1]
                ] += confusion_matrix(y_targ, y_pred, labels=labels)
            ...
            with open(
                join(self.config.out_dir, "accuracy_assessement.json"), "w"
            ) as fp:
                json.dump(res, fp)

            res = {
                d: {j: conf_matrix[d][j].tolist() for j in self.config.datasets}
                for d in self.config.datasets
            }
            with open(
                join(self.config.out_dir, "per_class_accuracy_assessement.json"), "w"
            ) as fp:
                json.dump(res, fp)

            PltPerClassMetrics()(
                conf_matrix, savefig=self.config.out_dir + "/per_class"
            )
