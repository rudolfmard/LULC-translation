...
class TranslatingUnetOSOten(nn.Module):
    def __init__(self, n_channels, n_classes, image_size, bias=False):
        ...
        self.down1 = Down(
            number_of_feature_map, 2 * number_of_feature_map, mode=down_mode, bias=bias
        )
        self.down2 = Down(
            2 * number_of_feature_map,
            4 * number_of_feature_map,
            factor=5,
            mode=down_mode,
            bias=bias,
        )
        self.down3 = Down(
            4 * number_of_feature_map, self.max_feature_map, mode=down_mode, bias=bias
        )
        self.down4 = Down(
            self.max_feature_map, self.max_feature_map, mode=down_mode, bias=bias
        )
        self.up1 = Up(
            16 * number_of_feature_map, 4 * number_of_feature_map, up_mode, bias=bias
        )
        self.up2 = Up(
            self.max_feature_map, 2 * number_of_feature_map, up_mode, bias=bias
        )
...
class TranslatingUnetfive(nn.Module):
    def __init__(self, n_channels, n_classes, image_size, bias=False):
        ...
        self.down2 = Down(
            2 * number_of_feature_map,
            4 * number_of_feature_map,
            mode="adaptativemaxpool",
            size=(image_size // 5, image_size // 5),
        )
        self.down3 = Down(
            4 * number_of_feature_map, self.max_feature_map, mode=down_mode, bias=bias
        )
...
class TranslatingUnettwo(nn.Module):
    def __init__(self, n_channels, n_classes, image_size, bias=False):
        ...
        self.up3 = Up(
            4 * number_of_feature_map, 2 * number_of_feature_map, up_mode, bias=bias
        )
        self.outc = nn.Conv2d(2 * number_of_feature_map, n_classes, kernel_size=1)
        self.sea_conv = DoubleConv(1, n_classes, bias=bias)
...
class TranslatingUnetsame(nn.Module):
    def __init__(self, n_channels, n_classes, image_size, bias=False):
        ...
        self.up3 = Up(
            4 * number_of_feature_map, number_of_feature_map, up_mode, bias=bias
        )
        self.up4 = Up(
            2 * number_of_feature_map, 2 * number_of_feature_map, up_mode, bias=bias
        )
        self.outc = nn.Conv2d(2 * number_of_feature_map, n_classes, kernel_size=1)
        self.sea_conv = DoubleConv(1, n_classes, bias=bias)
