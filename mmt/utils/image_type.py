...
class LandCover(ImageType):
    def __init__(
        ...
        if cmap:
            ...
            self.cmap = cmap
            self.matplotlib_cmap = LinearSegmentedColormap.from_list(
                cmap_name, np.array(cmap) / 255, N=n_bin
            )
        else:
...
class CLC(LandCover):
    def __init__(self, res=100, year=2018, level=3):
        ...
        id_labels = (
            np.unique(np.floor(np.array(id_labels) / (10 ** (3 - level))))
            .astype(np.int16)
            .tolist()
        )
    ...
    def __init__(self, kind="cover", year=2013):
        if kind == "cover":
            labels_name = [
                "Zones baties",
                "Zones non baties",
                "Zones a materiaux mineraux",
                "Zones a autres materiaux",
                "Sols nus",
                "Surfaces d'eau",
                "Neves et glaciers",
                "Peuplement de feuillus",
                "Peuplement de coniferes",
                "Peuplement mixte",
                "Formations arbustives et sous-arbrisseaux",
                "Autres formations ligneuses",
                "Formations herbacees",
...
class SRTM(Imagerie):
    def __init__(self, cmap=None):
        ...
        super(SRTM, self).__init__(
            producer, name, resolution, year, no_data_value, cmap=cmap
        )
