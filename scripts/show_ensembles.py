...
esawc = landcovers.ESAWorldCover(
    transforms=mmt_transforms.EsawcTransform, res=esgp.res / 6
)
esawc.res = esgp.res / 6
print(f"Landcovers loaded with native CRS and resolution")

translator = translators.EsawcToEsgpProba(
    checkpoint_path=checkpoint_path, always_predict=False
)
merger = translators.MapMergerProba(
    "", merge_criterion="qflag2_nodata", output_dtype="float32", tgeo_init=False
)
...
for i, domainname in enumerate(val_domains):
    ...
    print(
        ...
        + " ".join(
            [
                f"{x}.shape = {eval(x).get('mask').shape},"
                for x in ["x_esawc", "x_esgp", "x_esgml"]
            ]
        )
    )
    ...
    proba = merger.predict_from_data(
        torch.tensor(y), x_qflags["mask"].squeeze(), x_esgp["mask"].squeeze()
    )
...
cols = ["ECOSG-ML"] + [
    f"Member {mb + 1} (u={np.round(u_values[mb].item(),2)})" for mb in range(n_members)
]
